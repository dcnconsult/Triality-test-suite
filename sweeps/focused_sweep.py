...
Windows (editable defaults):
  g12 in [0.04, 0.05, 0.06]
  g13 in [-0.02, 0.0, 0.02]
  g23 in [-0.02, 0.0, 0.02]
  lam in [0.0, 0.01, 0.02]

Simulation defaults (higher fidelity):
  T = 1.5 s, dt = 7.5e-4, Nx = 96
Bootstrap:
  B = 30 resamples over time indices (block bootstrap optional).

Outputs:
  /mnt/data/triality_focused_results.csv
"""
import numpy as np
import pandas as pd
...
OUT = "/mnt/data/triality_focused_results.csv"
...
def bootstrap_time(Phi, dt, B=30, block=None, seed=42) -> Dict[str, Tuple[float,float,float]]:
    ...
    metrics = {"plv_low_12": [], "plv_mid_23": [], "plv_high_13": [],
...
def run(g12_vals=(0.04,0.05,0.06), g13_vals=(-0.02,0.0,0.02), g23_vals=(-0.02,0.0,0.02),
        lam_vals=(0.0,0.01,0.02),
        T=1.5, dt=7.5e-4, Nx=96, amp=0.03, discard_s=0.3, B=30, block=None):
    results = []
    for g12 in g12_vals:
        for g13 in g13_vals:
            for g23 in g23_vals:
                for lam in lam_vals:
                    ...
                    row = {
                        "g12": g12, "g13": g13, "g23": g23, "lam": lam,
                    }
                    for k, v in m.items():
                        row[k] = v
