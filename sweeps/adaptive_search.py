...
with finer steps. Writes a new CSV and a ranked summary.
"""
import os
import numpy as np
import pandas as pd
...
OUT = "/mnt/data/triality_adaptive_results.csv"
RANK = "/mnt/data/triality_adaptive_ranked.csv"

def objective(row, w_plv=1.0, w_pac=1.0):
    # Composite objective emphasizing both PLV (three bands) and PAC (two combos)
...
def metrics_from_sim(Phi, dt):
    """Compute PLV/PAC at center point, consistent with the sweep script."""
    from analysis.plv_pac import plv, pac_tort
    fs = 1.0/dt
...
def explore_local(seed, steps=(0.01, 0.01, 0.01, 0.01), span=2):
    """
    Build a fine local grid around a seed point (g12,g13,g23,lam).
    steps: step sizes for each param
    span: +/- number of steps to explore
    """
...
def run(csv_seed="/mnt/data/triality_sweep_results.csv",
        top_k=3, span=1, steps=(0.01,0.01,0.01,0.01),
            ...
            for g13 in g13_range:
                for g23 in g23_range:
                    for lam in lam_range:
                        ...
                        t,x,Phi,Vel = integrate_1d(mdl, L=1.0, Nx=Nx, T=T, dt=dt, c=1.0, drive=drive, controller=controller)
                        i0 = int(0.25/dt)
                        Phi_eff = Phi[i0:]
