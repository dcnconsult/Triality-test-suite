...
> **Version:** v11.1 (2025-01-03)  
> **Status:** Cross‑platform replication confirmed (JPC ↔ SPDC)  
> **Lead Artifact:** *Universality Report: Triadic Signatures in JPC and SPDC Systems* (auto‑generated by v11.1)

## The Triality Hypothesis: A Universal Mechanism?

The Triality Test Suite provides the software pipeline to investigate a compelling hypothesis: that a universal, triadic, phase-locked interaction underpins the dynamics of disparate quantum hardware. This toolkit was used to discover and validate a robust, instrument-agnostic signature of this interaction—a "Triality fingerprint"—present in both Josephson Parametric Converter (JPC) and Spontaneous Parametric Down-Conversion (SPDC) systems.

This finding, if upheld through further research, suggests new avenues for quantum control, error mitigation, and signal routing across superconducting and photonic platforms. We have open-sourced this toolkit to facilitate independent replication and to invite collaboration on this exciting frontier.

## Key Features
Validated Cross-Platform Analysis: The core of the toolkit is a set of analysis pipelines for identifying triadic interactions in both JPC and SPDC systems.

- Extensible & Modular: The analysis modules are well-structured, allowing for the easy integration of new hardware platforms, analysis techniques, or theoretical models.

- Reproducible by Design: The entire analysis pipeline, from data ingestion to report generation, is fully automated and can be replicated with a single script.

- Statistically Rigorous: The toolkit employs robust statistical methods, including surrogate data testing and multiple-comparison correction, to ensure the significance of its findings.

---

## Executive Summary
Triality Test Suite is a reproducible analysis pipeline for detecting **triadic, phase‑locked interactions** ("Triality fingerprint") in quantum systems. In v11.1, we validated **cross‑platform universality**: the same **narrow, statistically significant, rigidly phase‑locked bicoherence hotspot** appears in both a **Josephson Parametric Converter (JPC)** and **Spontaneous Parametric Down‑Conversion (SPDC)** optics. This strengthens the hypothesis that a **universal triadic mechanism** underlies disparate hardware.
...
> **Note:** Filenames above reflect v11.1 conventions. Earlier versions used similar names; see **Changelog** for deltas.
...
> See `reports/v11.1_universality.md` for figures: ridge plots, bicoherence heatmaps, hotspot masks, and effect‑size summaries.
...
- **v11.1 (2025‑01‑03)**: Windows PowerShell compatibility; fixed module imports; complete replication pipeline; cross-platform testing.
...
A: Edit `configs/*_v11.1.yaml` under `stats.rigidity_checks` and rerun.

**Q: Where are thresholds defined?**  
A: See `configs/report_v11.1.yaml` (IoU minimum, FDR q, Δbicoherence).

---

## Maintainers
- Darryl Novotny (Architecture & DSP)
